...
**Before starting work.** Claim your task atomically with vault.task-claim --as=<your-id>. If you see AlreadyClaimedError, another agent took it — surface that to your dispatcher and stop. Once claimed, post a single "ready to begin" message to your declared coordination channel using vault.channel-post so peers see you're live.

**During work.** Periodically call vault.channel-tail with since=<your-cursor> to pick up signals from peers. The cursor is the value returned by your previous tail call; persist it in session memory and pass it back next time. On cold start, pass the ISO timestamp of your task claim as since= and you will receive every message since you started.

**Ending work.** When your task is complete, post a ready signal to the channel in this format: branch=<branch-token> pr=<pr-number>. The merge orchestrator (Mewtwo) consumes these signals to build its merge queue. Then journal end-of-task with vault.agent-journal --channel=<channel> --entry="..." — the journal records what you did, which moves you used, and any open questions for follow-up agents.

**Vault-of-record caveat.** vault-mcp always reads from and writes to the main worktree's vault root. If you are running inside a per-agent git worktree, do NOT watch your worktree's filesystem for coordination signals — use vault.channel-tail. Your worktree filesystem is invisible to peers.`;
...
export function mcpToolName(toolName: ToolName): string {
  ...
  if (toolName.startsWith("vault.")) {
    const bare = toolName.slice("vault.".length);
    return `mcp__vault__vault_${bare}`;
  }
  return toolName;
}
